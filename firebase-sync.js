(function () {
  ...
  const META_KEYS = new Set([PENDING_KEY, LAST_SYNC_KEY, LAST_PULL_KEY, STATUS_KEY, STRUCTURE_KEY, LAST_ERROR_KEY, LOCAL_DIRTY_KEY, PACKAGE_META_KEY, DEVICE_ID_KEY]);
  ...
  function enqueueChange(key, value, operation, updatedAt = Date.now()) {
    ...
    queue.push({ key: normalized, value, operation, updatedAt });
    writeQueue(queue);
    ...
    if (navigator.onLine) {
      ...
      setWorkMode(isTeacherRole() ? "docente-contingencia" : "sin-internet");
      setStatus(isTeacherRole() ? "contingencia-local" : "pendiente-sin-internet");
    }
  }
  ...
  async function initFirebase() {
    ...
    try {
      ...
      console.info("Persistencia offline de Firestore no disponible:", error.code || error.message);
    }
  ...
  function queueAllLocalData() {
    const map = new Map(readQueue().map(item => [canonicalKey(item.key), { ...item, key: canonicalKey(item.key) }]));
    for (let i = 0; i < localStorage.length; i++) {
      const key = canonicalKey(localStorage.key(i));
      if (shouldSync(key) && canWriteKey(key)) {
        const updatedAt = packageUpdatedAt(key) || Date.now();
        touchPackageMeta(key, updatedAt);
        map.set(key, { key, value: rawGetItem(key), operation: "set", updatedAt });
      }
    }
  ...
  function queueTeacherCompensationIfNeeded() {
    ...
    if (!hasUncompensatedLocalChanges() && !readQueue().length) return false;

    const map = new Map(readQueue().map(item => [canonicalKey(item.key), { ...item, key: canonicalKey(item.key) }]));
    const updatedAt = Date.now();

    TEACHER_SYNC_KEYS.forEach(key => {
      if (shouldSync(key) && canWriteKey(key) && rawGetItem(key) !== null) {
        const keyUpdatedAt = packageUpdatedAt(key) || updatedAt;
        touchPackageMeta(key, keyUpdatedAt);
        map.set(key, { key, value: rawGetItem(key), operation: "set", updatedAt: keyUpdatedAt });
      }
    });
  ...
  function queueTeacherUsefulLocalDataIfQueueWasLost() {
    if (!isTeacherRole() || readQueue().length || !hasUncompensatedLocalChanges()) return false;
    const updatedAt = Date.now();
    const items = TEACHER_SYNC_KEYS
      .filter(key => shouldSync(key) && canWriteKey(key) && hasUsefulValue(key, rawGetItem(key)))
      .map(key => {
        const keyUpdatedAt = packageUpdatedAt(key) || updatedAt;
        touchPackageMeta(key, keyUpdatedAt);
        return { key, value: rawGetItem(key), operation: "set", updatedAt: keyUpdatedAt };
      });

    if (!items.length) return false;
    writeQueue(items);
    markLocalDirty(updatedAt);
    setStatus("compensacion-local-preparada");
    return true;
  }
  ...
  async function syncNow() {
    ...
    if (isViewerRole()) {
      writeQueue(readQueue().filter(item => canWriteKey(item.key)));
    ...
    try {
      ...
      const pending = [];
      const processing = new Map(queue.map(item => [canonicalKey(item.key), Number(item.updatedAt || 0)]));

      for (const item of queue) {
        if (!canWriteKey(item.key)) continue;
        if (isAdminRole() && !navigator.onLine) {
          pending.push(item);
          continue;
        }
        try { await writeItem(item); }
        catch (error) {
          setLastError(error, `subiendo ${item.key}`);
          pending.push(item);
        }
      }

      const latestQueue = readQueue();
      const lateChanges = latestQueue.filter(item => {
        const key = canonicalKey(item.key);
        const startedAt = processing.get(key);
        return startedAt === undefined || Number(item.updatedAt || 0) > startedAt;
      });
      const nextQueue = mergeQueueByLatest([...pending, ...lateChanges]);
      writeQueue(nextQueue);
      rawSetItem(STRUCTURE_KEY, STRUCTURE_VERSION);
      setStatus(nextQueue.length ? "pendiente" : "sincronizado");
      setWorkMode(nextQueue.length ? "pendiente" : (isTeacherRole() ? "docente-online" : "online"));
      if (!nextQueue.length) {
        rawSetItem(LAST_SYNC_KEY, String(Date.now()));
        rawRemoveItem(LOCAL_DIRTY_KEY);
      }
      retryPending = nextQueue.length > 0;
    } finally {
      syncing = false;
    }
    if (retryPending && navigator.onLine) scheduleSync(RETRY_SYNC_DELAY);
  }
  ...
  async function pullRemoteOnStart() {
    ...
    if (!currentRole() && hasLocalWorkBeforeLogin()) {
      setStatus("esperando-login-para-compensar");
      return false;
    }
    if (isTeacherRole() && (readQueue().length || hasUncompensatedLocalChanges())) {
      setStatus("pendiente-local-antes-de-descargar");
      return false;
    }
    try {
      setStatus("descargando");
      const force = isViewerRole() || !hasUsefulLocalData();
  ...
  function migrateLocalIfNeeded() {
    ...
    if (!navigator.onLine || rawGetItem(STRUCTURE_KEY) === STRUCTURE_VERSION || readQueue().length) return;
    if (!hasUsefulLocalData()) return;
    queueAllLocalData();
  }
  ...
  function scheduleSync(delay = null) {
    ...
    const wait = delay === null ? (readQueue().length ? RETRY_SYNC_DELAY : 900) : delay;
  ...
  window.firebaseAfterLogin = async function () {
    try {
      ...
      queueTeacherUsefulLocalDataIfQueueWasLost();
      queueTeacherCompensationIfNeeded();
      await syncNow();
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    try {
      ...
      booting = false;
      const role = currentRole();
      const hasLocalBeforeLogin = !role && hasLocalWorkBeforeLogin();
      normalizeCourseStorage(Boolean(role) && !isViewerRole());

      if (hasLocalBeforeLogin) {
        setStatus("esperando-login-para-compensar");
        return;
      }
  ...
  window.addEventListener("offline", () => {
    if (readQueue().length || isTeacherRole()) {
      setWorkMode(isTeacherRole() ? "docente-contingencia" : "sin-internet");
      setStatus(isTeacherRole() ? "contingencia-local" : "pendiente-sin-internet");
    }
  });
